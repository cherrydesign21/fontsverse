...
export default function AdModal({ onClose }: Props) {
  ...
  return (
    <Overlay onClose={onClose}>
      <h2 className="text-xl font-bold text-white mb-1">Post an Ad</h2>
      <p className="text-white/35 text-sm mb-5">Reach designers &amp; developers on FontsVerse</p>

      {!submitted ? (
        <>
          {/* Pricing note */}
          <div className="bg-violet-500/8 border border-violet-500/15 rounded-lg px-3 py-2.5 mb-4">
            <p className="text-violet-300 text-[12px] font-medium mb-1">📊 Ad Pricing</p>
            <p className="text-white/40 text-[11px]">Ads are reviewed manually. We'll contact you at your email to confirm pricing and go-live date.</p>
          </div>

          <label className="text-white/40 text-xs block mb-1">Ad Title *</label>
          ...
          <label className="text-white/40 text-xs block mb-1">Tagline</label>
          ...
          <label className="text-white/40 text-xs block mb-1">Destination URL *</label>
          <input className="fv-input mb-3" placeholder="https://yoursite.com" type="url"
            value={url} onChange={e => { setUrl(e.target.value); setErr(""); }} />

          <label className="text-white/40 text-xs block mb-1">Contact Email *</label>
          <input className="fv-input mb-3" placeholder="you@company.com" type="email"
            value={email} onChange={e => { setEmail(e.target.value); setErr(""); }} />

          <label className="text-white/40 text-xs block mb-1">Monthly Budget (USD)</label>
        ...
        <div className="text-center py-6">
          ...
          <p className="text-white font-bold text-lg mb-2">Request received!</p>
          <p className="text-white/40 text-sm mb-1">We'll review your ad and email you at</p>
          <p className="text-violet-400 text-sm font-mono mb-5">{email}</p>
          <p className="text-white/25 text-xs mb-5">Usually within 24 hours</p>
