...
export default function AccountModal({ onClose }: Props) {
  ...
  return (
    <Overlay onClose={onClose}>
      <h2 className="text-xl font-bold text-white mb-5">Account Settings</h2>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-white/35 text-xs">{profile.email}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <div className="bg-white/4 border border-white/8 rounded-lg p-3 text-center">
          <p className="text-xl font-black text-white">{fonts.length}</p>
          <p className="text-white/35 text-[11px] mt-0.5">Uploaded Fonts</p>
        </div>
        <div className="bg-white/4 border border-white/8 rounded-lg p-3 text-center">
          <p className="text-xl font-black text-white capitalize">{profile.role}</p>
          <p className="text-white/35 text-[11px] mt-0.5">Account Role</p>
        </div>
      </div>

      {isAdmin && (
        <div className="bg-amber-500/8 border border-amber-500/18 rounded-lg px-3 py-2 mb-4">
